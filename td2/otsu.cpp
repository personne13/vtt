...
void otsu (Mat &src_gray) {
  ...
  for (int i = 0; i < 255; i++)
  {
    ...
    std::cout << "manual otsu threshold t = " << best << std::endl;
...
process(const char* imsname)
{
  ...
  Mat diff;
  Mat src_gray_opencv = imread(imsname, CV_LOAD_IMAGE_GRAYSCALE);
	if(!src_gray.data){
  ...
  imshow("otsu-th", src_gray);
  imwrite("otsu-th.png", src_gray);
